<template>
  <div>
    ...
    <el-card class="box-card">
      ...
      <!-- 数据列表，data为表格绑定数据源studentList，内部所需的数据都会从studentList中做提取-->
      <el-table
        class="el-table-add"
        :data="studentList"
      ...
      >
        ...
        <el-table-column prop="department" label="专业" width="160">
        </el-table-column>
        ...
        <el-table-column prop="" label="操作">
          ...
          <template slot-scope="scope">
            ...
            <el-button
              ...
              icon="el-icon-edit"
              size="mini"
              @click="showEditDialog(scope.row.studentID)"
            ></el-button>
            <!-- 删除 -->
            <el-button
              type="danger"
              icon="el-icon-delete"
              size="mini"
              @click="removeUserById(scope.row.studentID)"
            ></el-button>
            <!-- 修改角色 文字提示组件-->
            <el-tooltip
              effect="dark"
    ...
    >
      ...
      >
        ...
        <el-form-item label="学号" prop="studentID">
          <el-input v-model="addFormData.studentID" ></el-input>
        </el-form-item>
        <!-- 学生-添加框-姓名 -->
        ...
        <el-form-item label="班级" prop="class">
          <el-input v-model="addFormData.class"></el-input>
        </el-form-item>
      </el-form>
...
import { postStudentID } from "../api/postStudentID.js";
import { deletStudent } from "../api/deletStudent.js";
...
export default {
  ...
  methods: {
    async getStudentList() {
      await getStudentData().then(response => {
        if (response.data.code === 200) {
          this.studentList = response.data.data;
          this.$message.success("查询成功");
        } else {
          this.$message.error("学生信息导入失败");
        }
      });
    },
    // 获取用户列表数据
    async getStudentList2() {
      // get请求方式要用params进行指定
      const { data: res } = await this.$http.get("/student_show", {
    ...
    addStudent() {
      this.$refs.addFormRef.validate(async valid => {
        ...
        const { data: res } = await addStudent({
          studentID: this.addFormData.studentID,
          name: this.addFormData.name,
          age: this.addFormData.age,
          sex: this.addFormData.sex,
          department: this.addFormData.department
        });
    ...
    async showEditDialog(ID) {
      // 根据ID查询用户信息
      const { data: res } = await postStudentID({ studentID: ID });
      console.log("查询当前id的信息");
      if (res.code !== 200) return this.$message.error("用户信息获取失败！");
    ...
    async removeUserById(ID) {
      ...
      // 否则发送删除用户的请求
      const { data: res } = await deletStudent({ studentID: ID });
